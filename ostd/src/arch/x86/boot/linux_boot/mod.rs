...
fn parse_kernel_commandline(boot_params: &BootParams) -> Option<&str> {
    if boot_params.ext_cmd_line_ptr != 0 {
        // We don't support the above 4GiB command line because it is not accessible through the
        // boot page table. We cannot log the error because the serial is not up. Proceed as if
