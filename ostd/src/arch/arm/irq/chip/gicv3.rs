...
impl Gic {
    pub(super) fn from_fdt(
        ...
        let mut next_reg = || {
            ...
            io_mem_allocator_builder.reserve(addr..addr + size)
        };
        ...
        let mut redistributor = {
            let io_mem = next_reg();
            Redistributor(DistributorBase { io_mem })
        };
    ...
    pub(super) fn map_interrupt_source_to(
        &self,
        interrupt_source: InterruptSourceInFdt<'_>,
        irq_line: &IrqLine,
        ...
        let intid = if is_spi {
            let intid = Distributor::BASE_SPI + id;
            ...
            let intid = Redistributor::BASE_PPI + id;
...
impl DistributorBase {
    ...
    unsafe fn set_priority(&mut self, intid: u32, prio: u8) {
        let offset = Self::GICD_IPRIORITYR + (intid as usize & !3);
        let shift = (intid & 3) * 8;
        unsafe {
    ...
    unsafe fn set_group1(&mut self, intid: u32) {
        let offset = Self::GICD_IGROUPR + (intid as usize / 32) * 4;
    ...
    unsafe fn set_edge_or_level(&mut self, intid: u32, is_edge: bool) {
        let offset = Self::GICD_ICFGR + (intid as usize / 16) * 4;
    ...
    unsafe fn set_enabled(&mut self, intid: u32, is_enabled: bool) {
        let offset = if is_enabled {
            Self::GICD_ISENABLER + (intid as usize / 32) * 4
        } else {
            Self::GICD_ICENABLER + (intid as usize / 32) * 4
        };
        let bit = 1u32 << (intid % 32);
