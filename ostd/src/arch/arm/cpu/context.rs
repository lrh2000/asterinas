...
impl UserContext {
    ...
    ///
    /// Currently, we only allow the setting or clearing of arithmetic condition flags and states
    /// for Software Step and Illegal Execution. Any other bits will be ignored and will remain
    /// unchanged.
    pub fn set_process_state(&mut self, pstate: usize) {
        // Be careful. We can only allow bits to be set that won't affect soundness.
        const ALLOWED_BITS: usize =
        ...
        self.user_context.spsr = (self.user_context.spsr & !ALLOWED_BITS) | (pstate & ALLOWED_BITS);
    }
